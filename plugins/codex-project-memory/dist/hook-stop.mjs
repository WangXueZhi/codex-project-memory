...
function turnHasToolUse(transcriptPath, turnId) {
  ...
  for (const line of readFileSync5(transcriptPath, "utf8").split(/\r?\n/)) {
    ...
    try {
      ...
      const eventTurnId = payload?.turn_id ?? payload?.internal_chat_message_metadata_passthrough?.turn_id;
      if (eventTurnId !== turnId || event.type !== "response_item") continue;
      if (payload?.type && ["custom_tool_call", "function_call", "mcp_tool_call", "command_execution"].includes(
        payload.type
      )) {
        return true;
      }
...
function hasValidReceipt(message) {
...
function evaluateStopHook(input, service) {
